...
class HDF5Plot(pg.PlotCurveItem):
    def __init__(self, *args, **kwds):
        self.hdf5 = None
        self.my_data_x = np.linspace(0, 5, 10000)
        self.inc = self.my_data_x[2] - self.my_data_x[1]
    ...
    def updateHDF5Plot(self):
        ...
        start2 = int(xrange[0]//self.inc) - 1
        start = min(0, np.abs(start2))
        stop = min(len(self.hdf5), int(xrange[1]//self.inc + 2))
        print(start)
        print(stop)
        ...
        my_data_x = np.linspace(0, 10, len(visible))

        self.setData(x=my_data_x, y=visible)  # update the plot
...
my_data_y = np.random.normal(3, 3, size=10000)
my_data_x = np.linspace(0, 5, 10000)
